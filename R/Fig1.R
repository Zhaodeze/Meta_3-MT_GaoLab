...
ggplot(Impact_Pathway,aes(x = Impact,y = log10P,color = group)) +
  ...
  scale_color_manual(values = c("grey", "#5F9EA0")) +
...
####  Fig1G  #######################
####################################
HCC_N_data <- Plasma_3MT %>% filter(Group %in% c("HCC","Normal"))

library(ggplot2)
library(ggpubr)
library(ggplot2)
ggplot(HCC_N_data, aes(x = Group, y = Meta_3MT, fill = Group)) +
  geom_violin(trim = FALSE, color = "black", linewidth = 0.6) +
  geom_jitter(color = "black",width = 0.15,size = 1,alpha = 0.6) +
  stat_summary(
    fun.data = function(x) {
    data.frame(y = median(x),ymin = quantile(x, 0.25),ymax = quantile(x, 0.75))},
    geom = "crossbar",width = 0.35,color = "#1C1C1C",linewidth = 0.7) +
  scale_fill_manual(values = c("HCC" = "#d0564f","Normal" = "#50aabb")) +
  scale_y_log10() +
  theme_classic(base_size = 13) +
  labs(x = "",y = expression("Meta-3MT"))
